...
def _source_ref(page: Any, table: Any, row: int, col: int) -> dict[str, Any]:
    ...
    if isinstance(bbox, (list, tuple)) and len(bbox) == 4:
        ref["bbox"] = list(bbox)
    confidence = getattr(cell, "confidence", None) if cell is not None else None
        ...
        for table in getattr(page, "tables", None) or ():
            ...
            for row_index, row in enumerate(rows):
                ...
                for column, field in labels:
                    ...
                    if label_count <= 1 and column + 1 < len(row) and str(row[column + 1] or "").strip():
                        choices.append((str(row[column + 1]), row_index, column + 1))
                    ...
                    if label_count > 1 and column + 1 < len(row) and str(row[column + 1] or "").strip():
                        choices.append((str(row[column + 1]), row_index, column + 1))
                    for raw, value_row, value_col in choices[:1]:
...
def extract_candidate_b_profile(context: Any) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {}
    for field, raw_candidates in _table_candidates(context.pages).items():
        ...
        refs = [ref for candidate in candidates for ref in candidate.get("source_refs") or ()]
